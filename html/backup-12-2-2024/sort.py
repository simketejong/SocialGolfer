...
def tempBuggy(players):
    random.shuffle(players)
    for player in players[:8]:
        player.buggy = True
    pro_players = [player for player in players if not player.buggy]
    random.shuffle(pro_players)  # Shuffle again to ensure randomness
    for player in pro_players[:3]:
        player.pro = True

def organize_flights(players, total_days):
    # Initialize the schedule with empty lists for each day and flight
...
def ZoekOplossing():
    ...
    print(organize_flights(players, 5))
